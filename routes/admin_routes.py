        ...
        if request.method == 'POST':
            ...
            if action == 'add_key':
                ...
                if new_api_key:
                    ...
                    if api_key:
                        if api_key.is_valid:
                            flash(f'Thêm API key "{key_name}" thành công và key đang hoạt động tốt', 'success')
                        else:
                            flash(f'Đã thêm API key "{key_name}" nhưng có vấn đề: {api_key.status_message}', 'warning')
                else:
                ...
                key_id = request.form.get('key_id')
                if key_id and key_id.isdigit():
                    if api_key_manager.delete_api_key(int(key_id)):
                        flash('Xóa API key thành công', 'success')
                    else:
                        flash('Không thể xóa API key', 'error')
                else:
